...
internal class SnailNumberTest {
    ...
    fun exampleInput(): List<SnailNumber.Node> {
        return exampleInputString.map { it.parseSnailNumber() }
    }

    @Test
    fun `example solution 1`() {
        val nums = exampleInput()
    ...
    @Test
    fun `can generate permutations of any list`() {
        val result = listOf(1, 2, 3, 4).permutations()
    ...
    fun `example solution 2`() {
        val pairs = exampleInputString.permutations()
        val maxMagnitude = findMaxMagnitude(pairs)

        assertThat(maxMagnitude).isEqualTo(3993)
    }

    @Test
    fun `puzzle solution 2`() {
        val pairs = puzzleInput.permutations()
        val maxMagnitude = findMaxMagnitude(pairs)

        assertThat(maxMagnitude).isEqualTo(4671)
    }

    private fun findMaxMagnitude(pairs: List<Pair<String, String>>) =
        pairs.maxOfOrNull { (f, s) ->
            val f1 = f.parseSnailNumber()
            val s1 = s.parseSnailNumber()

            val f2 = f.parseSnailNumber()
            val s2 = s.parseSnailNumber()

            val m1 = listOf(f1, s1).snailSum().reduce().magnitude
            val m2 = listOf(s2, f2).snailSum().reduce().magnitude
            val m = maxOf(m1, m2)
            m
        } ?: error("Could not find maximum magnitude")

    fun <T> List<T>.permutations(): List<Pair<T, T>> {
        val res = LinkedList<Pair<T, T>>()
        var todo = this
        while (todo.size > 1) {
            val head = todo.first()
            todo = todo.drop(1)
            val h = generateSequence(head) { it }
            val pairs = h.asIterable().zip(todo)
            res += pairs
        }
        return res
    }
    ...
    fun puzzleInput(): List<SnailNumber.Node> {
        return puzzleInput.map { it.parseSnailNumber() }
    }
}