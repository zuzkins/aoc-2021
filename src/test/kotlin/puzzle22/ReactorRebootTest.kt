...
import puzzle22.ReactorReboot.parseCuboid
import puzzle22.ReactorReboot.process
import puzzle22.ReactorReboot.volume
...
internal class ReactorRebootTest {
    ...
    @Test
    fun `can parse cuboid expression`() {
        val c = parseCuboid("on x=10..12,y=10..12,z=10..12")
        assertThat(c.toggle).isTrue
        assertThat(c.x).isEqualTo(10..12)
        assertThat(c.y).isEqualTo(10..12)
        assertThat(c.z).isEqualTo(10..12)
    }

    @Test
    fun `can parse cuboid expression 2`() {
        val c = parseCuboid("off x=9..11,y=9..11,z=9..11")
        assertThat(c.toggle).isFalse
        assertThat(c.x).isEqualTo(9..11)
        assertThat(c.y).isEqualTo(9..11)
        assertThat(c.z).isEqualTo(9..11)
    }

    @Test
    fun `can parse cuboid expression 3`() {
        val c = parseCuboid("on x=-46..7,y=-6..46,z=-50..-1")
        assertThat(c.toggle).isTrue
        assertThat(c.x).isEqualTo(-46..7)
        assertThat(c.y).isEqualTo(-6..46)
        assertThat(c.z).isEqualTo(-50..-1)
    }

    @Test
    fun `can compute range intersection`() {
        assertThat((10..12).intersection(11..20)).isEqualTo(11..12)
    }

    @Test
    fun `cube can be toggled`() {
        val cube = Cube(-50, -50, -50, 100)

        val cuboid = parseCuboid("on x=10..12,y=10..12,z=10..12")
        cube.process(cuboid)
        assertThat(cube.getTurnedOn()).hasSize(27)
        assertThat(cube.getTurnedOn()).hasSameElementsAs(
            listOf(
                Triple(10, 10, 10),
                Triple(10, 10, 11),
                Triple(10, 10, 12),
        ...
        cube.process(parseCuboid("on x=11..13,y=11..13,z=11..13"))
        assertThat(cube.getTurnedOn()).hasSize(46)
        cube.process(parseCuboid("off x=9..11,y=9..11,z=9..11"))
        assertThat(cube.getTurnedOn()).hasSize(38)
        cube.process(parseCuboid("on x=10..10,y=10..10,z=10..10"))
        assertThat(cube.getTurnedOn()).hasSize(39)
    }
    ...
    val example = """
        ...
        """.trimIndent().lines().map { parseCuboid(it) }

    @Test
    fun `can process example`() {
        val cube = Cube(-50, -50, -50, 101)
        example.forEach { c ->
            cube.process(c)
        }
        assertThat(cube.getTurnedOn()).hasSize(590784)
    }

    @Test
    fun `can solve puzzle 1`() {
        val cube = Cube(-50, -50, -50, 101)
        puzzleInput.forEach { c ->
            cube.process(c)
        }
        assertThat(cube.getTurnedOn()).hasSize(588120)
    }

    @Test
    @Disabled
    fun `can solve example by shifting cube`() {
        val cuboids = listOf(parseCuboid("on x=10..12,y=10..12,z=10..12"))
        val counter = processCuboids(cuboids, -50, 50, 3)
        assertThat(counter).isEqualTo(27)
        assertThat(processCuboids(cuboids, -50, 50, 1000)).isEqualTo(27)
    }

    @Test
    @Disabled
    fun `can solve puzzle by shifting cube`() {
        assertThat(processCuboids(puzzleInput, -100000, 100000, 300)).isEqualTo(27)
    }

    @Test
    fun `can compute differential intersection of 2 cuboids - on and off - 1`() {
        val cuboidOn = parseCuboid("on x=1..5,y=1..5,z=1..5")
        val cuboidOff = parseCuboid("off x=4..5,y=4..5,z=4..5")
        assertThat(cuboidOn.intersect(cuboidOff)).isEqualTo(Some(parseCuboid("off x=4..5,y=4..5,z=4..5")))
    }

    @Test
    fun `can compute differential intersection of 2 cuboids - on and off - 2`() {
        val cuboidOn = parseCuboid("on x=1..5,y=1..5,z=1..5")
        val cuboidOff = parseCuboid("off x=4..15,y=4..15,z=4..15")
        assertThat(cuboidOn.intersect(cuboidOff)).isEqualTo(Some(parseCuboid("off x=4..5,y=4..5,z=4..5")))
    }

    @Test
    fun `can compute differential intersection of 2 cuboids - on and off - 3`() {
        val cuboidOn = parseCuboid("on x=1..15,y=1..15,z=1..15")
        val cuboidOff = parseCuboid("off x=4..5,y=4..5,z=4..5")
        assertThat(cuboidOn.intersect(cuboidOff)).isEqualTo(Some(parseCuboid("off x=4..5,y=4..5,z=4..5")))
    }

    @Test
    fun `can compute differential intersection of 2 cuboids - on and off - 4`() {
        val cuboidOn = parseCuboid("on x=1..5,y=1..5,z=1..5")
        val cuboidOff = parseCuboid("off x=14..15,y=14..15,z=14..15")
        assertThat(cuboidOn.intersect(cuboidOff)).isEqualTo(None)
    }

    @Test
    fun `can compute differential intersection of 2 cuboids - on and on - 1`() {
        val cuboidOn = parseCuboid("on x=1..15,y=1..15,z=1..15")
        val cuboidOff = parseCuboid("on x=4..5,y=4..5,z=4..5")
        assertThat(cuboidOn.intersect(cuboidOff)).isEqualTo(Some(parseCuboid("off x=4..5,y=4..5,z=4..5")))
    }

    @Test
    fun `can compute differential intersection of 2 cuboids - off and on - 1`() {
        val cuboidOn = parseCuboid("off x=1..15,y=1..15,z=1..15")
        val cuboidOff = parseCuboid("on x=4..5,y=4..5,z=4..5")
        assertThat(cuboidOn.intersect(cuboidOff)).isEqualTo(Some(parseCuboid("on x=4..5,y=4..5,z=4..5")))
    }

    @Test
    fun `can compute volume of a cuboid`() {
        assertThat(parseCuboid("on x=10..12,y=10..12,z=10..12").volume).isEqualTo(27)
        assertThat(parseCuboid("off x=9..11,y=9..11,z=9..11").volume).isEqualTo(-27)
    }

    @Test
    fun `can process create of differential cuboids`() {
        val cuboids = listOf(parseCuboid("on x=10..12,y=10..12,z=10..12"), parseCuboid("on x=11..13,y=11..13,z=11..13"))
        val processedCuboids = cuboids.process()
        assertThat(processedCuboids).hasSize(3)
        assertThat(processedCuboids).isEqualTo(
            listOf(
                Cuboid(x = 10..12, y = 10..12, z = 10..12, toggle = true),
                Cuboid(x = 11..12, y = 11..12, z = 11..12, toggle = false),
                Cuboid(x = 11..13, y = 11..13, z = 11..13, toggle = true)
            )
        )
        assertThat(processedCuboids.volume).isEqualTo(46)
    }

    @Test
    fun `can process create of differential cuboids - 2`() {
        val cuboids = listOf(
            parseCuboid("on x=10..12,y=10..12,z=10..12"),
            parseCuboid("on x=11..13,y=11..13,z=11..13"),
            parseCuboid("off x=9..11,y=9..11,z=9..11")
        )
        val processedCuboids = cuboids.process()
        assertThat(processedCuboids).hasSize(6)
        assertThat(processedCuboids).isEqualTo(
            listOf(
                Cuboid(x = 10..12, y = 10..12, z = 10..12, toggle = true),
                Cuboid(x = 11..12, y = 11..12, z = 11..12, toggle = false),
                Cuboid(x = 11..13, y = 11..13, z = 11..13, toggle = true),
                Cuboid(x = 10..11, y = 10..11, z = 10..11, toggle = false),
                Cuboid(x = 11..11, y = 11..11, z = 11..11, toggle = true),
                Cuboid(x = 11..11, y = 11..11, z = 11..11, toggle = false)
            )
        )
        assertThat(processedCuboids.volume).isEqualTo(38)
    }

    @Test
    fun `can process create of differential cuboids - 3`() {
        val cuboids = listOf(
            parseCuboid("on x=10..12,y=10..12,z=10..12"),
            parseCuboid("on x=11..13,y=11..13,z=11..13"),
            parseCuboid("off x=9..11,y=9..11,z=9..11"),
            parseCuboid("on x=10..10,y=10..10,z=10..10")
        )
        assertThat(cuboids.process().volume).isEqualTo(39)
    }

    @Test
    fun `can solve example by cuboids differences`() {
        assertThat(bigExample.process().volume).isEqualTo(2758514936282235)
    }

    @Test
    fun `can solve puzzle by cuboids differences`() {
        assertThat(puzzleInput.process().volume).isEqualTo(1134088247046731)
    }

    private fun processCuboids(cuboids: List<Cuboid>, from: Int, to: Int, size: Int): Long {

        val steps = ceil((abs(from) + abs(to) + 1) / size.toDouble()).toInt()

        var counter = 0L
        var cube = Cube(from, from, from, size)

        (0..steps).forEach { z ->
            println("${Instant.now()} Step Z: $z")
            (0..steps).forEach { y ->
                // println("${Instant.now()} Step Y: $y, Z: $z")
                (0..steps).forEach { x ->
                    cube = cube.shift(from + x * size, from + y * size, from + z * size)
                    cuboids.forEach { cube.process(it) }
                    counter += cube.getTurnedCount()
                }
            }
        }
        return counter
